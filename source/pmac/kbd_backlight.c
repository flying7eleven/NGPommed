...
static int kbd_backlight_get( void ) {
	return kbd_bck_info.level;
}

/* Helper for LMU-controlled keyboards */
static void lmu_write_kbd_value( int fd, unsigned char val ) {
	unsigned char buf[ 3 ];
...
static void kbd_lmu_backlight_set( int val, int who ) {
	int curval;
...
static void adb_write_kbd_value( int fd, unsigned char val ) {
	int ret;
...
static void kbd_pmu_backlight_set( int val, int who ) {
	int curval;
...
static void kbd_backlight_set( int val, int who ) {
...
void kbd_backlight_step( int dir ) {
	int val;
	int newval;
...
static int kbd_get_i2cdev( void ) {
	char buf[ PATH_MAX ];
	int i2c_bus;
	int ret;
