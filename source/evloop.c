...
int evloop_add( int fd, uint32_t events, pommed_event_cb cb ) {
	int ret;
...
static void evloop_timer_callback( int fd, uint32_t events ) {
	uint64_t ticks;
...
static int evloop_create_timer( int timeout ) {
	int fd;
	int ret;
...
int evloop_add_timer( int timeout, pommed_timer_cb cb ) {
	int fd;
...
int evloop_remove_timer( int id ) {
	int found;
	int ret;
